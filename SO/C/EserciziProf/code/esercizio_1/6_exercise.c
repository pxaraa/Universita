/* 
  ...
  Il programma deve terminare quando la somma dei due numeri ricevuti supera il valore 190: 
  il padre, allora, invia un segnale di terminazione a ciascuno dei figli.   
*/
#include <stdio.h>
#include <signal.h>
#include <unistd.h>
...
int main(){
  ...
  if (pid1==0) {
     ...
     while(1) {
       ...
       if (n%2==1) {
         write(pd1[SCRIVI],&n,sizeof(n)); // Everything is a file!! Ma allora e' vero...
       }
     }
  }
  else if (pid1>0) {
    // Codice del padre
    // SNODO FONDAMENTALE: SOLO IN QUESTO PUNTO IL PADRE PUO' GENERARE IL SECONDO PROCESSO
    pid2=fork();
    if (pid2==0) {
